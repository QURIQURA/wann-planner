 ...
 *
 * Each stage keeps its own fixed colour (see CAKE_STAGES) so this row reads
 * as a legend: a hollow ring means nothing is tagged for that stage yet (a
 * gap), a solid dot in that stage's colour means at least one tagged Task is
...
/**
 * Compact inline picker: a small coloured dot that opens a dropdown of
 * CAKE_STAGES to tag/retag a single Task's stage. Shown next to a Project
...
export function StageDot({
  ...
  const color = value ? stageColor(value) : null;
  const label = value ? CAKE_STAGES.find((s) => s.key === value)?.label ?? value : "단계 없음";
